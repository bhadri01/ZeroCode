  ...
  return (
    <header className="zc-sh" id={anchor}>
      <style>{`
        ...
        .zc-sh .k::before { content: ''; width: 28px; height: 1px; background: linear-gradient(90deg, transparent, var(--accent)); }
        .zc-sh h2 {
          font-family: var(--f-display); font-weight: 400;
          font-size: clamp(40px, 4.6vw, 60px); line-height: 1.05;
          letter-spacing: -0.018em;
          margin: 22px 0 16px; color: var(--fg); text-wrap: balance;
        }
        .zc-sh h2 .it {
          font-style: italic;
        ...
        .zc-sh p { color: var(--fg-2); max-width: 620px; font-size: 17px; margin: 0; line-height: 1.6; }
      `}</style>
  ...
  return (
    <section className="zc-trust">
      <style>{`
        ...
        .zc-trust-cell .u { font-family: var(--f-mono); font-size: 11px; color: var(--fg-2); letter-spacing: 0.1em; text-transform: uppercase; }
        .zc-trust-cell .n { font-size: 12px; color: var(--fg-3); margin-top: 1px; }
...
interface WhyPoint { k: string; v: string }
...
function WhyCard({ no, label, title, body, points, glyph }: WhyCardProps) {
  ...
  return (
    <article className="zc-why" ref={ref}>
      <style>{`
        ...
        .zc-why ul { list-style: none; padding: 0; margin: 0; font-family: var(--f-mono); font-size: 12px; color: var(--fg-1); }
        .zc-why ul li { display: flex; gap: 12px; padding: 9px 0; border-top: 1px dashed var(--line); }
        ...
        .zc-why .glyph {
          ...
          transition: transform .35s ease, opacity .25s ease; z-index: 1;
        }
  ...
  return (
    <section className="zc-why-sec">
      <style>{`
        ...
        .zc-why-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
...
type TabId = 'compose' | 'helm' | 'binary';
interface DeployLine { html: string }

export function DeploySection() {
  const [tab, setTab] = useState<TabId>('compose');
  const [copied, setCopied] = useState(false);
  const blocks: Record<TabId, (DeployLine | null)[]> = {
    compose: [
      { html: '<span class="dim"># clone, configure, run — 60 seconds to first /v1/submissions</span>' },
      { html: '<span class="pr">$</span> <span class="cmd">git clone</span> https://github.com/bhadri01/ZeroCode' },
  ...
  const plain: Record<TabId, string> = {
    compose: 'git clone https://github.com/bhadri01/ZeroCode\ncd ZeroCode && cp .env.example .env\ndocker compose up -d\ncurl localhost:8080/v1/health',
  ...
  function copy() {
    try { navigator.clipboard.writeText(plain[tab]); setCopied(true); setTimeout(() => setCopied(false), 1200); } catch { /* noop */ }
  }
  ...
  return (
    <section className="zc-deploy">
      <style>{`
        ...
        .zc-deploy-tab.active { background: color-mix(in oklab, var(--accent) 8%, var(--bg-1)); color: var(--fg); }
        .zc-deploy-tab.active::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 2px; background: var(--accent); box-shadow: 0 0 14px var(--accent); }
        .zc-deploy-tab .n { font-family: var(--f-mono); font-size: 13px; }
        .zc-deploy-tab .s { font-size: 11.5px; color: var(--fg-3); margin-top: 2px; }
        .zc-deploy-code { border: 1px solid var(--line-2); border-radius: 12px; background: var(--bg-1); overflow: hidden; box-shadow: 0 30px 80px -30px rgba(0,0,0,0.6); }
        .zc-deploy-code-hd { padding: 11px 14px; border-bottom: 1px solid var(--line); display: flex; align-items: center; gap: 10px; font-family: var(--f-mono); font-size: 11px; color: var(--fg-3); letter-spacing: 0.08em; text-transform: uppercase; }
        ...
        .zc-deploy-code-hd .copy { margin-left: auto; cursor: pointer; color: var(--fg-2); padding: 3px 8px; border: 1px solid var(--line-2); border-radius: 4px; transition: color .15s ease, border-color .15s ease, background .15s ease; background: transparent; font-family: var(--f-mono); font-size: 10px; letter-spacing: 0.08em; }
  ...
  return (
    <footer className="zc-footer">
      <style>{`
        ...
        .zc-footer .mark { font-family: var(--f-mono); font-size: 18px; font-weight: 500; letter-spacing: -0.01em; margin-bottom: 12px; display: flex; align-items: center; gap: 10px; }
        .zc-footer .mark .g { width: 20px; height: 20px; position: relative; }
        .zc-footer .mark .g::before, .zc-footer .mark .g::after { content: ''; position: absolute; inset: 0; border: 1.5px solid currentColor; }
        ...
        .zc-footer h5 { font-family: var(--f-mono); font-size: 11px; color: var(--fg-3); letter-spacing: 0.16em; text-transform: uppercase; margin: 0 0 14px; }
        .zc-footer ul { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 8px; }
        .zc-footer ul a { color: var(--fg-1); font-size: 13.5px; transition: color .15s ease, padding-left .15s ease; display: inline-block; }
